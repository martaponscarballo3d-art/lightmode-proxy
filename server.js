...
function compactPrompt(text, max = 1950) {
  if (!text) return '';
  ...
  return cleaned.slice(0, max - 100).trim() +
    " Full realistic jewelry piece, centered, physically coherent.";
}
...
app.post('/img2img', async (req, res) => {
  ...
  (async () => {
    try {
      ...
      const promptFinal = compactPrompt(`
${prompt}

Realistic luxury jewelry product render. Full piece visible, centered, not cropped.
Premium metal, coherent real jewelry construction.
Keep reference recognizable in bracelet layout, chain style, scale and central composition.
Client changes are mandatory: replace reference name/details with requested name and charms.
`, 1950);
      ...
      form.append('image_strength', String(strength || 0.26));
